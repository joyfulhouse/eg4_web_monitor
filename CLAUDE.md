...
### Current Version
- **v3.4.0-beta.14** — Offline inverter no longer blacks out all entities (#256): an offline device (cloud `lost:true`) returns a partial runtime/battery payload; pylxpweb's `InverterRuntime`/`BatteryInfo` declared the omitted live fields required, so validation failed → `has_data=False` → every entity (incl. Status) unavailable while an online sibling worked. Fix = pylxpweb 0.9.36b13 makes cloud-omittable fields Optional + battery-bank aggregates None-safe; offline device now shows `Status=offline`. Requires pylxpweb>=0.9.36b13.
