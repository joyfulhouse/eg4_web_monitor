...
`_attr_has_entity_name = True`. The 17 `_attr_entity_id` assignments in the
codebase are **inert**: `homeassistant.helpers.entity.Entity.entity_id` is a plain
class attribute, not `_attr_`-backed, so HA ignores them (issue #550).
...
`grep -rhcE '^\s*(async )?def test_' tests/ | paste -sd+ | bc`. Coverage target
>95%. Fixtures come from
