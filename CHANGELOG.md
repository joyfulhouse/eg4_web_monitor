...
- **Example dashboards re-audited against real entity IDs** ([#209](https://github.com/joyfulhouse/eg4_web_monitor/issues/209)): every entity reference in `examples/dashboards/` (`battery_details.yaml`, `energy_overview.yaml`, `eg4_solar_monitor.yaml`) was validated against a live 3.4.0-beta entity registry and fixed. This re-applies the v3.2.0 renames from #212 (which were lost when `main` was superseded by the 3.3.0 release branch) and catches the 3.3.0/3.4.0 changes: dropped the phantom `eg4_` entity-ID prefix (sensors are `sensor.<model>_<serial>_*`), `battery_soc` → `state_of_charge`, `pv_power` → `pv_total_power`, `daily_*` → `yield`/`consumption`/`grid_import`, inverter `load_power` → `consumption_power`, per-battery `state_of_charge` → `relative_soc` and `cell_voltage_max/min` → `max/min_cell_voltage`, battery suffixes now use the `battery_<serial>_<nn>` device (`real_power`, `state_of_health`, `cell_temperature_delta`, `max/min_voltage_cell_number`), `eg4_gridboss_*` → `grid_boss_*`, switches `battery_backup` → `eps_battery_backup` and `peak_shaving_mode` → `grid_peak_shaving_mode`, and never-shipped controls remapped to real entities (`grid_charge` → `ac_charge_mode`, `feed_in_grid` → `forced_discharge_mode`, `battery_high/low_soc_limit` → `system_charge_soc_limit`/`on_grid_soc_cut_off`). Dashboards now also showcase the v3.4.0 **Battery Charge/Discharge Control** selects, and headers explain the placeholder convention.
