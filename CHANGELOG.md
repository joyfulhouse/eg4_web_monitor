...
- **Cloud connections only.** This control writes through the EG4 cloud, exactly as the portal does. Unlike most switches it has no local Modbus path and is **not created on Local-only or Hybrid connections**, where its state cannot be read at all. The reason is deliberate: the function lives somewhere in holding register 179, but *which bit* has never been confirmed on hardware. Writing a guessed bit is not a safe failure — the firmware acknowledges the write either way, so the cloud fallback would never trigger and a read-back check could not detect the mistake. If someone captures a raw register 179 toggle from the portal, the bit can be pinned and the local path added; until then the cloud is the only route that is known to hit the right thing.
