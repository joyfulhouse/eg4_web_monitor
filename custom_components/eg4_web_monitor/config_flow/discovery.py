...
# Model mapping from device type code to (model_name, inverter_family)
_DEVICE_TYPE_MODEL_MAP: dict[int, tuple[str, str]] = {
...
def _get_model_from_device_type(device_type_code: int) -> tuple[str, str]:
    ...
    """
    return _DEVICE_TYPE_MODEL_MAP.get(
        device_type_code, ("Unknown", INVERTER_FAMILY_PV_SERIES)
    )
...
async def discover_dongle_device(
    ...
    try:
        ...
        device = await _read_device_info_from_transport(transport, inverter_serial)
