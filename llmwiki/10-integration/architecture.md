...
| Control writes reach the device by **two** mechanisms. Most go through one shared router — local first, then cloud **where a cloud client exists** (HYBRID; a LOCAL-only entry has no cloud leg). Two controls bypass that router. A **second mechanism** hands a pylxpweb method name to the library, which then chooses the transport itself — eg4's fallback, link-down short-circuit and cache seeding are absent there, and it is invisible to a grep of the coordinator's write primitives | `verified-against-code` (`utils.py` → `async_write_with_cloud_fallback`; `base_entity.py` → `_execute_switch_action`). The frame and both derivations: [controls-and-writes.md §0](controls-and-writes.md#0-two-write-mechanisms-not-one) |
