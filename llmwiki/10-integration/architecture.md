...
| Most control writes go through one shared router (`utils.py` → `async_write_with_cloud_fallback`), but **not all do**, and the set of write shapes is not enumerable from this page. Do not reason about write routing from here | `verified-against-code` — [controls-and-writes.md §0](controls-and-writes.md#0-the-write-surface-is-not-reliably-enumerable-from-documentation) owns the shapes, the derivation and the known blind spots |
