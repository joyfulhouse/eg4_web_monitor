...
verified-against:
  ...
  # #592 (2026-09-02, PR branch pollux/issue-592-peak-shaving): the
  # PeakShavingNumber rows in §0.4/§1/§6/§10 describe that change set and
  # are licensed per claim by their inline #592 citations until the
  # release-cut re-pin moves the mainline pin past the merge.
  eg4_web_monitor: 1a2c24e
  pylxpweb: c78ab7e
...
Line numbers are pinned per repo by the `verified-against:` mapping above — `e146d91` for
`eg4_web_monitor`, `ab87902` for `pylxpweb`. Between the previous pins (`0e2366f` / `aafc4e3`)
and these, the only cited source files that changed are eg4 `coordinator_mixins.py` (the #560
temperature-blanking merge shifted `_perform_dst_sync`, re-numbered below) and pylxpweb
`constants/registers.py` (comment-only change, not line-cited here); every other cited file is
byte-identical, so its line numbers carry over unchanged.
...
| **d** | An entity-scoped search misses **background writes** | `coordinator_mixins.py:4559` → `_perform_dst_sync` calls `station.sync_dst_setting()` hourly. `dst_sync_enabled` defaults to **`True`** (`coordinator.py:408`). No entity is involved |
...
| `PeakShavingNumber` (five spec rows, [#592](https://github.com/joyfulhouse/eg4_web_monitor/issues/592)) | **router** — `_write_parameter` with `local_value=` in engineering units, `cloud_write=` the named holdParam writer with readback, `local_write_blocked_reason=_offgrid_cloud_only_reason` | `write_named_parameter` (local, the transport applies ×10 for the DIV_10 rows) / `write_parameter` (cloud). Never `set_grid_peak_shaving_power`, never `_write_voltage_register` | H232 (PS2 power), H207/H218 (SOC 1/2), H208/H219 (voltage 1/2) |
...
> `0.9.36b6`. At the pinned `ab87902` the name resolves, so the entity takes the router branch
...
| **Voltage registers** | Local writes by **name**; cloud writes by **raw register address** | Asymmetric on purpose. **Exception ([#592](https://github.com/joyfulhouse/eg4_web_monitor/issues/592)):** the peak-shaving voltage rows H208/H219 do **not** use `_write_voltage_register` — their pylxpweb name carries a DIV_10 transport scaling, so they go through `_write_parameter` with plain volts and the cloud named writer (§10 #10) | `number.py:513-525`; `PeakShavingNumber.async_set_native_value` |
...
| 2 | `FUNC_SMART_LOAD_ENABLE` has **no pinned bit** (179 bit 13 is a placeholder) → Smart Load stays cloud-only. `FUNC_ON_GRID_ALWAYS_ON` is **pinned** at H179 b15 (`hardware-toggle-proven` 2026-08-12 on FlexBOSS21 SYNTH00003 — portal toggle flipped local raw 0x1048→0x9048, XOR exactly 0x8000, clean restore verified cloud+local; see [registers.md](../40-hardware/registers.md) H179 safe bit map for the scope and the `app-write-path-proven` lineage that carries the family-wide extension) and is writable locally on the shipped pylxpweb floor (≥0.9.39b11 carries the mapping) | `verified-against-code` — Smart Load gate `switch.py` setup; Grid Always On `_WORKING_MODE_PARAMETERS` + `_local_params_can_carry()`; GH #559 |
...
| 10 | **A DIV_10-scaled pylxpweb name must be written in engineering units on BOTH paths.** For `_12K_HOLD_GRID_PEAK_SHAVING_{POWER_2,VOLT,VOLT_2}` the transport's named read divides the raw register by 10 and its named write multiplies back (`LOCAL_PARAM_SCALE_DIV10`), so an entity that pre-scales (decivolts via `_write_voltage_register`, or `int(value)` from `_write_parameter`'s default) lands raw ×100 (52.0 V → 5200) or truncates (4.5 kW → 4). The cloud readback for these rows must compare as floats within half a step — the int-truncating comparison reads 4 for a 4.5 write and fails a write that landed | `verified-against-code` — pylxpweb `c78ab7e` `transports/protocol.py` read scaling and `write_named_parameters`; eg4 `PeakShavingNumber` + `_write_cloud_named_parameter` (`_FLOAT_READBACK_TOLERANCE`), [#592](https://github.com/joyfulhouse/eg4_web_monitor/issues/592) change set; tests `TestPeakShavingLocalWrites` / `TestPeakShavingCloudWrites` |
