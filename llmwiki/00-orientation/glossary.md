...
sources:
  ...
  - pylxpweb src/pylxpweb/transports/protocol.py
  - pylxpweb src/pylxpweb/constants/registers.py
  - llmwiki/README.md
verified-against:
...
> Every pylxpweb symbol cited below was re-checked at commit `204b95d`, not at a working
> copy. The pin is the commit: a tag is a mutable ref and cannot constrain what it points
> at later, so no tag name appears here.
...
| **Placeholder key** | A synthetic `FUNC_<reg>_BIT<n>` name for an unidentified bit. These were once reachable by named write, meaning a write could read-modify-write an unknown bit; local named writes to them are refused by name pattern, so they are decode-only. **The guard keys on the name, not on the evidence** — a bit that carries a *real* name but an unpinned position is not covered by it, which is exactly how H179 b11 ships a local write (see **Semantic proof** above). The companion denylist for named-but-disputed parameters, `DISPUTED_WRITE_BLOCKED_PARAMS`, is an empty frozenset: a name belongs there only while a dispute is live. Both refusals live in the **local named-parameter** write path and nowhere else — and that scope is narrower than "local writes". They sit inside pylxpweb's `write_named_parameters`, so a write that never passes a parameter *name* never meets them: the register-numbered `write_parameters` entry point carries no such check (this is the path behind a raw register write), and a switch action that awaits a pylxpweb inverter method by name does not route through `write_named_parameters` either. **Do not read this guard as coverage of the local write surface**; it covers one mechanism within it. | `verified-against-code` for both guards (pylxpweb `transports/protocol.py` → `write_named_parameters`: `_PLACEHOLDER_PARAM_RE = re.compile(r"FUNC_\d+_BIT\d+")` at `:25`, applied by `fullmatch` at `:562`; `DISPUTED_WRITE_BLOCKED_PARAMS` imported at `:547` and tested at `:569`, defined empty at `constants/registers.py:1046`) — and for their local-only scope, since at `204b95d` neither symbol occurs anywhere outside those two files. `asserted-unverified` for **why** that scope is considered safe — that the cloud `functionControl` path cannot land on the wrong bit because EG4's server resolves the name — which is a design rationale recorded in a `constants/registers.py` comment, not something any symbol enforces. Origin of the hazard: issue #476, `memory/issue-476-green-mode-bit14.md` |
...
| **Semantic proof** (of a bit or register meaning) | The standard that a delta test does *not* meet. All four are required: (1) a named vendor or UI action on the target family, (2) an **independent observation that the intended physical state actually changed** — a second sensor, a breaker test, a portal widget that reflects the real mode — (3) a complete raw before/after delta on the register, and (4) restoration of the original value. Anything less leaves the semantic **unproven**. An unproven bit **must** then be treated as write-inaccessible — no entity, no named-write path, no placeholder key a write helper can reach. **That is the required safety policy. It is not a description of this codebase, and nothing in the code enforces it** — no gate consults a register's evidence grade before writing it, and many production entities write unpinned mappings on the local path. [README's ladder](../README.md#the-rule-is-not-enforced-anywhere-in-the-code) gives the procedure for deriving the current local-write surface, plus a dated snapshot; this entry deliberately keeps no list of its own, because every curated copy of that list has turned out to be **incomplete when written** — not stale later, but wrong at the commit it was written against. The risk is tracked on issue **#558** and [contradiction C7](../60-history/open-contradictions.md). **Never infer from a weak grade that a write path is closed; check the entity.** Which grade the evidence earns is the legend's call, never this entry's. Per-bit status is the keeper's, [`40-hardware/registers.md`](../40-hardware/registers.md). | `asserted-unverified` for the standard (issue #476; `memory/issue-476-green-mode-bit14.md`). The shipped write paths are described and graded in README's ladder |
