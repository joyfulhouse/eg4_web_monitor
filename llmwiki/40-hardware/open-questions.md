...
| What are the still-unknown H233 bits, and which families implement them? | See the [H233 map](registers.md#h233-safe-bit-map) and [off-grid access boundary](registers.md#h233-off-grid-access-boundary); LOCAL rejection and applicability remain family-dependent. | `asserted-unverified` | Capture one named action per bit with the complete proof tuple, explicitly preserve the raw request and exception response for any family rejection, and preserve atomic read-modify-write around the shared word. |
...
Closing an item requires updating [registers.md](registers.md) with the tested family, raw evidence, restore result where writable, and the weakest defensible grade. Record component firmware as scope metadata when available; if it is unrecorded, limit the conclusion to the tested unit. A unit fixture cannot close a hardware question.
