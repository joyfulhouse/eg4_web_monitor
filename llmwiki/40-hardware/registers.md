...
| Shipped routing (since PR #569, 2026-08-13): the Quick Charge switch **fails closed** — only a positively resolved non-off-grid family keeps the local H233 route; pure-LOCAL off-grid/unresolved units get an unavailable switch, and a forced service call raises before pylxpweb is reached. With a cloud client, the off-grid family goes cloud-direct. | current integration | `verified-against-code` | current | [PR #569](https://github.com/joyfulhouse/eg4_web_monitor/pull/569) (merged; `switch.py` → `_prefers_cloud_control` / `is_positively_non_offgrid_family`, `_offgrid_without_cloud` availability gate). This superseded the earlier shipped-path exposure this section recorded — pure-LOCAL off-grid toggles no longer attempt the rejected write. Evidence sweep that closed the derivation: [issue #570](https://github.com/joyfulhouse/eg4_web_monitor/issues/570). |
