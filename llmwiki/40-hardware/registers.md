...
verified-against:
  # Both pins moved to mainline at the 2026-08-12 release cut. e146d91 is the
  # eg4 #562 merge (Grid Always On in LOCAL/HYBRID) — every eg4 line citation
  # on this page was re-checked there and drifted switch.py/device_types.py
  # anchors re-numbered. ab87902 is pylxpweb mainline (the 0.9.39b11 release
  # commit; the #559 bit-15 pin merged as 9c10a07 and is contained in it),
  # re-pinned from the squash-merged PR #270 branch head aafc4e3.
...
| **Current total** | **336** | **32 proven (9.5%)** |
...
| H227 | System charge SOC limit, 0-101% | tested 18kPV | `hardware-toggle-proven` | current on tested unit; cross-family write risk unresolved | 1 | Named System Charge SOC Limit action and raw 80→101→80 restoration; component firmware version unrecorded — scope limited to the tested unit. `memory/soc-charge-limit-101-top-balance.md`. Shipped-path fact (`verified-against-code`): `_create_number_entities` adds `SystemChargeSOCLimitNumber` to the always-on block for every supported inverter reaching it, with no family gate; `async_set_native_value` sends `PARAM_HOLD_SYSTEM_CHARGE_SOC_LIMIT` through the local-first router. HYBRID may fall back to cloud; LOCAL has no cloud fallback. Code anchors: [`number.py::_create_number_entities`](../../custom_components/eg4_web_monitor/number.py#L697), [`SystemChargeSOCLimitNumber.async_set_native_value`](../../custom_components/eg4_web_monitor/number.py#L815), and [`utils.py::async_write_with_cloud_fallback`](../../custom_components/eg4_web_monitor/utils.py#L185). Family-suppression fact (`verified-against-code`): no `is_family_control_supported` call guards this row, and that helper fails open elsewhere—“a device whose family is missing or unknown keeps every control”—while [`INVERTER_FAMILY_UNKNOWN`](../../custom_components/eg4_web_monitor/const/device_types.py#L55) is the truthy string `"UNKNOWN"`. See [`utils.py::is_family_control_supported`](../../custom_components/eg4_web_monitor/utils.py#L165). Safety conclusion (`inferred`): the shipped entity does not enforce the tested-unit evidence scope; do not assume this mapping is established or a local write is safe on FlexBOSS21, 12kPV, or another untested target. Code-side escalation: [issue #558](https://github.com/joyfulhouse/eg4_web_monitor/issues/558#issuecomment-5232409553). |
