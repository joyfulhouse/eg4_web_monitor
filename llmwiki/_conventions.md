...
| `sources` | yes | Everything the page was written from. Repo-relative paths, absolute paths for out-of-repo sources, or URLs. |
...
- In prose, put it in backticks at the end of the sentence, with its citation:
...
- If you cannot corroborate what a source asserts, the grade is `asserted-unverified`
  and you **name the source**. That is a complete, acceptable answer. Silently promoting
  it to `inferred` is not.
